...
export default function UserCatalog(props) {
    ...
    useEffect(() => {
        setFilterSongs(props.songs.filter((song) => {
    ...
    const tBodies = filterSongs.map((song, index) => {
        ...
        const songsRows = songValues.map((value, i) => {
            ...
            const editLink = i === 0 ? <button rowSpan={songValues.length + 1} value={song.id} onClick={() => navigate(`/catalog/${song.id}/`)}>EDIT</button> : null
            return (
                <tr key={i}>
                    {songTitle}
                    {songArtist}
